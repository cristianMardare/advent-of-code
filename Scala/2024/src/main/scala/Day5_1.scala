...
case class Day5_1() extends Challenge[String, Int] {
  ...
  private def split(in: List[String]): (List[String], List[String]) =
    (
      in.filter(s => s.contains("|")),
      in.filter(s => s.contains(","))
    )

  //noinspection ScalaDeprecation
  private def processConditions(in: List[String]): (Map[Int, Set[Int]], Map[Int, Set[Int]]) = {
    val pattern = "(\\d*)\\|(\\d*)".r
    val pairs: List[(Int, Int)] = in.map {
      case pattern(n1, n2) => (n1.toInt, n2.toInt)
    }

    @tailrec
    def processRec(pairs: List[(Int, Int)], keyFn: ((Int, Int)) => Int, valueFn: ((Int, Int)) => Int, out: Map[Int, Set[Int]]): Map[Int, Set[Int]] = {
      pairs match {
        case Nil => out
        case head::tail => out get keyFn(head) match {
          case None => processRec(tail, keyFn, valueFn, out + (keyFn(head) -> Set(valueFn(head))))
          case Some(s) => processRec(tail, keyFn, valueFn, out + (keyFn(head) -> Set(s.toSeq.appended(valueFn(head)) *)))
        }
      }
    }

    val pagesAfter = processRec(pairs, (in1, in2) => in1, (in1, in2) => in2, Map[Int, Set[Int]]())
    val pagesBefore = processRec(pairs, (in1, in2) => in2, (in1, in2) => in1, Map[Int, Set[Int]]())
